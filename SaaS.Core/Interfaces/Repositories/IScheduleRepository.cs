...
public interface IScheduleRepository
{
    ...
    Task<IReadOnlyList<Schedule>> GetAllAsync(DateTime? fromUtc, DateTime? toUtc, int? assignedUserId, int? clientId, CancellationToken ct = default);
    ...
    // Time entries
    Task<int> ClockInAsync(int scheduleId, int userId, decimal? latitude, decimal? longitude, CancellationToken ct = default);
    Task<int> ClockOutAsync(int scheduleId, int userId, decimal? latitude, decimal? longitude, CancellationToken ct = default);
    ...
    Task ApplyAutoClockPhasesAsync(DateTime nowUtc, bool autoClockIn, bool autoClockOut, CancellationToken ct = default);

    // Client visits + signatures
    Task<IReadOnlyList<ClientVisit>> GetByClientAsync(int clientId, DateTime? fromUtc, DateTime? toUtc, string? status, CancellationToken ct = default);
    Task<int> CreateSignatureAsync(TimeEntrySignature signature, CancellationToken ct = default);
    Task<IReadOnlyList<TimeEntrySignature>> GetSignaturesAsync(int timeEntryId, CancellationToken ct = default);
